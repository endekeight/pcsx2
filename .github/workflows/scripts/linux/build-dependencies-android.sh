...
f117507dc501f2a6c11f9241d8d0c3213846cfad91764361af37befd6b6c523d  ffmpeg-n$FFMPEG.tar.gz
EOF
...
curl -L \
	...
	-o "ffmpeg-n$FFMPEG.tar.gz" "https://github.com/FFmpeg/FFmpeg/archive/refs/tags/n$FFMPEG.tar.gz" \
...
# Headers only. USE_LINKED_FFMPEG is OFF on Android, so GSCapture dlopen()s the library
# at runtime and nothing links against it - but since upstream cf8d05f4a dropped the
# in-tree 3rdparty/ffmpeg/include copy, the headers have to come from somewhere.
rm -rf "FFmpeg-n$FFMPEG"
tar xf "ffmpeg-n$FFMPEG.tar.gz"
cd "FFmpeg-n$FFMPEG"
for lib in libavcodec libavformat libavutil libswresample libswscale; do
	mkdir -p "$INSTALLDIR/include/$lib"
	find "$lib" -maxdepth 1 -name '*.h' -exec cp {} "$INSTALLDIR/include/$lib/" \;
done
# configure generates this one; it carries exactly two public defines and we know both
# for arm64-v8a, so writing it beats running ffmpeg's configure for headers we never build.
cat > "$INSTALLDIR/include/libavutil/avconfig.h" <<'AVCONFIG'
